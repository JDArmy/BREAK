...
**英文翻译**：`src/i18n/en/BREAK/risks/{ID}.json`（只放可翻译文本，如 title, keywords, definition, description, complexity, influence, references[].title, references[].link）
...
**英文翻译**：`src/i18n/en/BREAK/terms/{ID}.json`（仅包含 title, aliases, keywords, definition, description, usageExample, references[].title, references[].link）
...
**英文翻译**：`src/i18n/en/BREAK/cases/{ID}.json`（仅包含 title, keywords, summary, description, references[].title, references[].link）
...
当需要新增一个实体类型时（如第 7 个类型），需要修改以下位置：

1. `src/BREAK/{entity}/` — 数据目录 + index.ts 聚合
2. `src/BREAK/index.ts` — 导入并展开到 BREAK 主对象
3. `src/validation/breakSchema.ts` — Zod schema
4. `src/utils/entityRoute.ts` — `inferEntityType` / `I18N_COLLECTION` / `DETAIL_ROUTE_MAP` / `ENTITY_ID_PATTERN`
5. `src/router/index.ts` — 列表路由 + 详情路由
6. `src/views/{Entity}View.vue` — 视图组件
7. `src/composables/useSearch.ts` — `FUSE_CONFIGS`
8. `src/composables/useEntityResolver.ts` — `TYPE_LABEL_KEY` / `FIELD_PRIORITY`
9. `src/components/entity/autoLinkerCore.ts` — `INTERACTIVE_SELECTORS`（若有新的交互元素选择器）
10. `src/i18n/en/BREAK/{entity}/` — 英文翻译目录
11. 验证脚本（`scripts/validate/`）— 新增实体校验

如果频繁新增类型（一年 3+ 个），考虑提取 Entity Registry 模式——将步骤 4/7/8 的映射集中到 `src/registry/entityRegistry.ts`，消费方从注册表派生。当前 6 个类型手动维护成本可控，暂不需要。
...
- 当 `src/BREAK/` 下的任何 JSON 文件发生修改（新增、编辑、删除条目）时，必须同步更新 `src/i18n/en/BREAK/` 下对应文件中的**文本翻译字段**（如 title, description, definition, complexity, influence, references[].title, references[].link；具体字段以对应实体实际 schema 为准）。
- 英文翻译文件只需包含可翻译的文本字段，结构数据（ID 数组、updated 等）会在运行时从中文源自动合并。英文翻译文件也维护 `references[].link`，以支持未来英文独立参考链接。
