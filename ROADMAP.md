...
> 文档版本：2.0 | 最后 Review：2026-06-13 | 当前项目版本：2.8.0 | 状态：基础能力已成型，下一阶段重点是可信内容、分析能力、维护效率和性能基线。
...
| 分析能力 | 关系网络和攻击路径已可视化，但还缺少可回答问题的统计视图 | 支持覆盖率、路径、类别和场景层面的分析 |
...
参考资料总量：795 条。当前 `npm run validate:data` 可通过 Schema 校验和中英文 key 同步检查。
...
| 数据维护 | 本地 editor 路由和 Express 保存服务，保存前已有部分校验 |
...
| 数据关系覆盖审计 | 新增 `scripts/validate/relations.mjs` 和 `npm run audit:relations`，输出关系覆盖 Markdown/JSON 报告 | ✅ |
| 参考资料质量基线 | `scripts/validate/references.mjs` 已按 error/review/warning 分级，并保留低质量域名、重复链接、i18n 链接差异报告 | ✅ |
...
| 依赖审计 | 新增 `DEPENDENCY_AUDIT.md`，记录实际使用依赖和待确认依赖 | ✅ 初版完成，是否删除待确认依赖留给后续小 PR |

### Phase B：轻量分析页（高优先，2-4 周）

目标：从“能浏览知识”升级到“能回答覆盖和分布问题”。不建设重型 BI，只做当前 JSON 可稳定计算的视图。

| 任务 | 说明 | 验收标准 |
|------|------|----------|
| 新增 `/dashboard` 路由 | 独立分析页，避免首页继续堆内容 | 导航可进入，移动端可读 |
| 实体概览与趋势占位 | 展示 Risk/Avoidance/AttackTool/ThreatActor/BusinessScene 数量和主/子条目比例 | 与数据统计脚本一致 |
| 风险复杂度分布 | 初级/中级/高级数量和占比 | 图表点击可筛选或跳转到风险列表 |
| 规避分类分布 | AC01-AC04 数量、覆盖率和未分类异常提示 | 可跳转到规避手段筛选视图 |
| 关系覆盖分析 | Risk-Avoidance、AttackTool-Risk、ThreatActor-Tool/Risk 覆盖情况 | 明确显示空值、弱覆盖和高关联节点 |
| 业务场景覆盖分析 | 各 BusinessScene 覆盖的风险数量和重复度 | 可跳转到首页对应业务场景 |
| i18n 与主题适配 | 图表、tooltip、legend 跟随语言和主题 | 中英文和暗色模式均可读 |
...
| 优先级 | 项目 | 原因 |
|--------|------|------|
| P0 | 依赖审计后续清理、参考资料 review/warning 降噪 | Phase A 初版已完成，后续要处理报告中的具体问题 |
| P1 | 轻量 Dashboard、图谱详情面板 | 直接提升分析价值 |
| P1 | 编辑器体验收敛、贡献模板 | 降低维护成本 |
...
| 指标 | 当前状态 | 下一目标 |
|------|----------|----------|
| 数据校验 | Schema + i18n sync 通过 | CI/Deploy 强制执行 |
| 测试 | 49 个用例 | 关键路由和图谱数据转换增加单元测试 |
| 关系覆盖 | 缺少独立报告 | 每次数据变更可生成覆盖报告 |
| 参考资料 | 795 条，结构统一 | 可输出质量分级和问题清单 |
| Lighthouse | 未建立基线 | 建立桌面/移动端基线 |
| Bundle | 有 `npm run analyze` | 设置主 chunk 和 RelationView chunk 观察阈值 |
| 文档一致性 | README 存在旧统计 | README、ROADMAP、数据统计脚本一致 |
