...
> 文档版本：2.7 | 最后 Review：2026-06-17 | 当前项目版本：2.14.2 | 状态：Phase A/B/C 基线能力已完成，Phase D 静态数据、npm 数据包评估、贡献模板和变更日志规范已完成初版；移动端 Lighthouse 已完成两轮首屏减负，当前主要剩余项是继续压低移动端 Sankey TBT 和长期内容映射。
...
| Lighthouse | 已建立桌面/移动端基线；当前预取策略避免移动端首屏过早拉取全部知识页，同时保留 PC 点击响应速度；搜索弹窗、英文 BREAK 数据、首页详情抽屉和移动端关系页 ECharts 首屏加载已拆分/延后 | 继续降低移动端 Sankey TBT，并逐步提高无障碍分数 |
