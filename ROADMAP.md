...
> 文档版本：2.7 | 最后 Review：2026-06-17 | 当前项目版本：2.14.2 | 状态：Phase A/B/C 基线能力已完成，Phase D 静态数据、npm 数据包评估、贡献模板和变更日志规范已完成初版；移动端 Lighthouse 已完成首轮首屏减负，当前主要剩余项是进一步压低移动端 LCP/TBT 和长期内容映射。
...
| 性能基线 | ECharts 和全量数据已进入项目，已有测量基线但移动端 LCP 仍偏高 | 持续提高 Lighthouse 移动端体验和运行时稳定性 |
...
| 性能 | PC 端保留知识页和关系图积极预取；移动端搜索弹窗、关系页 ECharts 和次级视图预加载已按需/延迟处理，降低首屏主线程竞争 |
...
| 优先级 | 项目 | 原因 |
|--------|------|------|
| P1 | Lighthouse 优化 | Lighthouse 已建立桌面/移动端基线；已调整为 PC 立即预取、移动端延迟/按需预取，并拆出移动端搜索弹窗与关系页 ECharts 首屏加载，后续继续减少移动端首页和 Sankey 页 LCP/TBT |
...
| Lighthouse | 已建立桌面/移动端基线；当前预取策略避免移动端首屏过早拉取全部知识页，同时保留 PC 点击响应速度；搜索弹窗和移动端关系页 ECharts 首屏加载已拆分/延后 | 逐步提高移动端 LCP/TBT 和无障碍分数 |
