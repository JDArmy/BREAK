...
> 文档版本：2.1 | 最后 Review：2026-06-13 | 当前项目版本：2.8.0 | 状态：Phase B 首批分析与基线能力已落地，下一阶段继续增强关系解释、Lighthouse 和数据产品化。
...
参考资料总量：742 条。当前 `npm run validate:data` 可通过 Schema 校验、中英文 key 同步检查和关系覆盖审计；`npm run audit:metrics` 可生成指标基线报告。
...
| 分析看板 | `/dashboard` 展示实体规模、参考资料覆盖、复杂度分布、规避分类、关系覆盖、业务场景覆盖和高关联节点 |
...
| 实体概览 | 展示 Risk/Avoidance/AttackTool/ThreatActor/BusinessScene 数量和主/子条目比例 | ✅ |
| 风险复杂度分布 | 初级/中级/高级数量和占比 | ✅ |
| 规避分类分布 | AC01-AC04 数量和占比 | ✅ |
| 关系覆盖分析 | Risk-Avoidance、AttackTool-Risk、ThreatActor-Tool/Risk 覆盖情况，显示空值、弱覆盖和高关联节点 | ✅ |
| 业务场景覆盖分析 | 各 BusinessScene 覆盖的风险数量和重复度 | ✅ |
| i18n 与主题适配 | 图表、tooltip、legend 跟随语言和主题 | ✅ |
| 图表联动跳转 | 分布图点击后带筛选条件跳转到实体列表 | 待做 |
...
| P1 | 图谱详情面板、路径解释、Dashboard 图表联动 | 直接提升分析价值 |
...
| 关系覆盖 | `audit:relations` 和 `audit:metrics` 可输出报告 | 关系变更 PR 必跑 |
