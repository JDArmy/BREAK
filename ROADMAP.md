...
> 文档版本：2.7 | 最后 Review：2026-06-17 | 当前项目版本：2.14.2 | 状态：Phase A/B/C 基线能力已完成，Phase D 静态数据、npm 数据包评估、贡献模板和变更日志规范已完成初版；当前主要剩余项是移动端 Lighthouse 优化和长期内容映射。
...
| 优先级 | 项目 | 原因 |
|--------|------|------|
| P1 | Lighthouse 优化 | Lighthouse 已建立桌面/移动端基线；已调整为 PC 立即预取、移动端延迟/按需预取，后续继续减少移动端首页和 Sankey 页 LCP/TBT |
...
| Lighthouse | 已建立桌面/移动端基线；当前预取策略避免移动端首屏过早拉取全部知识页，同时保留 PC 点击响应速度 | 逐步提高移动端 LCP 和无障碍分数 |
