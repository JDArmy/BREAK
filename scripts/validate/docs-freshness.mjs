...
const hasAny = (predicates) => changedFiles.some((file) => predicates.some((predicate) => predicate(file)));
...
const readmeTriggers = [
  ...
  (file) => file.startsWith("scripts/") && /export|package|stix|jsonld|bundle|lighthouse|smoke|performance/.test(file),
  (file) => file === "DATA_SCHEMA.md",
...
const gates = [
  {
    ...
    reason: "UI、路由、数据模型、验证脚本、构建链路或维护流程发生变化时，需要同步更新使用手册。",
  },
  {
    name: "README",
    active: hasAny(readmeTriggers),
    required: ["README.md", "README_CN.md"],
    reason: "公共命令、构建/发布门禁、导出产物、数据模型或 Skill 分发方式发生变化时，需要同步更新 README。",
  },
  {
    name: "Skill 文档",
    active: hasAny(skillTriggers),
    required: ["SKILL.md", "SKILL_en.md"],
    reason: "Skill 搜索脚本、导出数据、实体结构或 Skill 打包方式发生变化时，需要同步更新 Skill 文档。",
  },
];
...
for (const gate of activeGates) {
  console.log(`- ${gate.name}: ${gate.missing.length ? "missing" : "ok"} (${gate.reason})`);
}
