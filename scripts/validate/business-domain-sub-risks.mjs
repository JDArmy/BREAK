/**
 * business-domain-sub-risks.mjs
 *
 * 校验（默认）+ 修复（--fix）BusinessDomain 中的子风险重复问题。
 *
 ...
 *
 * 用法：
 *   node scripts/validate/business-domain-sub-risks.mjs          # 校验模式
 *   node scripts/validate/business-domain-sub-risks.mjs --fix     # 修复模式
 */
...
/** 加载所有 BD 文件，返回 [{ filePath, bdId, data }] */
function loadBusinessDomainFiles() {
  return fs
    ...
    .sort()
    .map((f) => {
      const filePath = path.join(bdDir, f);
      const data = readJson(filePath);
      const bdId = Object.keys(data)[0];
      return { filePath, fileName: f, bdId, data };
    });
}
...
function run() {
  ...
  for (const { filePath, fileName, bdId, data } of files) {
    const domain = data[bdId];
    const riskScenes = domain.riskScenes || {};
  ...
  if (totalDuplicates === 0) {
    console.log('✅ BusinessDomain 子风险校验通过（无父子重复）');
    return;
  }
  ...
  if (totalDuplicates > 0) {
    if (fix) {
      ...
      console.error('运行 node scripts/validate/business-domain-sub-risks.mjs --fix 自动修复');
    }
  }
