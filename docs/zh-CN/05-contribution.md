...
- **详细描述（`description`）** ≥ 40 字。perception（感知）/ detection（识别）必须命中检测信号词（采集端：采集/埋点/指纹/日志/流量；判定端：阈值/规则/模型/基线）。仅写「检测/识别」等自指词不达标。
- **局限性（`limitation`）** 必填，中文 30-200 字，英文不超过 150 个单词，不得占位套话。perception/detection 必须含「被绕过方式」（绕过/破解/伪造/模拟）或「误报场景」（误报/漏报/误判/误伤）。
