...
1. **内容与引用治理仍需闭环**：review 152 + timeout 150 + connection_error 66 条问题链接仍需按域名策略分批复核；高价值案例 primary source 覆盖率为 59.04%（442 个缺口），其中 criminal_verdict 类仍是最大短板。
...
已完成：审计工具链已建成（`case-source-quality.mjs`），可按类别统计高价值案例 primary 覆盖率；当前全量覆盖率 55.20%，高价值覆盖率 60.52%，并已将 `view.inews.qq.com` 归入 secondary 来源以减少 unknown 噪声。

未完成工作：
- 高价值 1,079 案例中 426 个缺 primary source。按类别分批补源：
  - `criminal_verdict`：49.25%（304 个缺口，最大短板），优先法院、检察院、公安、监管通报。
