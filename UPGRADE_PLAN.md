...
- 修正 `C0838` 引用错误——原引用 secrss/69385 指向国安部间谍案例，与案例描述的陕西警方侵犯公民个人信息案不匹配。替换为央视网《法治在线》实际报道，incidentTime 修正为 2024-07。
- 续批已补强 `C0770`（央视《朝闻天下》tv.cctv.com/上海除夕夜智能电表遭攻击案）、`C0496`（最高人民法院案例汇编 PDF/gipc.gov.cn）共 2 条高价值案例 primary 来源；`case-source-quality.mjs` 新增 `gipc.gov.cn` 为 primary 域名。多子代理复核中，`C0087`/`C0209`/`C0218`/`C0530` 公安/法院/检察院通告 gov.cn 原始页面已下线或需登录、`C0391`/`C0566` 案件仅有地方报纸或侦查阶段、`C0600`/`C0662`/`C0664` 仅有中新网/澎湃（secondary）、`C0686`/`C0688`/`C0778` 人民网/检察日报已下线/扬子晚报（secondary），均暂不硬补。
