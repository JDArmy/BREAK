...
替换 T0621 清结算术语 references 为央行条法司权威链接

## 2.46.4

修复英文文档图表并补全数据出口路径

- 实体自动链接器跳过 SVG、样式、脚本和 Mermaid 图表容器，避免英文术语匹配改写 Mermaid 内联 CSS 并导致节点、连线样式失效。
- 补充 Mermaid SVG 回归测试，验证图表样式文本和容器内容不会参与实体自动链接。
- 为中英文数据消费指南补齐 STIX 2.1 与 JSON-LD 文件的完整公开 URL。

## 2.46.3

修复代码审查发现的安全与门禁重跑问题

- LLM 客户端默认使用 HTTPS，移除对指定 HTTPS 端点的静默 HTTP 降级，并统一为进程环境变量优先于本地 `.env`。
- 修正文档正文链接在相对 `BASE_URL` 和 GitHub Pages 子目录部署下无法被前端路由接管的问题。
- LLM 门禁跳过记录改为按受影响实体 key 执行全量强制重评，并补齐 `review:changed` 对 `--force` 的子评审透传。
- 新增 LLM 配置与文档子目录链接回归测试，同步中英文 README、贡献指南和发布维护说明。

## 2.46.2

修正历史评审发现的实体分类与案例关系

- 完成历史 LLM 门禁遗留的 references、field-density、classification 与 case-fact 评审，清零调用失败和阻断性结论。
- 修正 2 个 Avoidance、2 个 Case 与 26 个 Term 的分类，使检测、黑产生态、数据采集、洗钱、身份安全、应用安全等实体边界与统一分类枚举一致。
- 修正 6 个 Case 的风险与攻击工具关系，消除 MEV、侧信道、工业安全、人格权等场景的错挂关联。
- 关闭历史 LLM 门禁重跑记录，清理可再生成的本地 research 审计目录。

## 2.46.1

清零P2实体抽取待办并补充结构化候选治理

- 新增 MCP、后量子密码学、加密敏捷性、TWAP、熔断机制、命令与控制、双重治理、滑点、Purdue 模型、数据二极管、特权账号、灵魂绑定代币、账户抽象、内存池、延迟注入和域间隙 16 个中英文术语实体。
- `review:should-extract` 升级为仅接收具有明确名称、类型和独立定义边界的结构化候选，过滤实现步骤、组件能力、协议参数、同义词和过细子技术。
- 完成剩余 P2 抽取建议的新增或已有实体覆盖判定，当前有效质量待办清零；同步 LLM 失败检查点与恢复重跑文档。
- 刷新首页轻量计数、README 与 Skill 统计，数据校验和 625 项测试全部通过。
...
行业术语分类语义 key 化并建立集中注册表与门禁

- 将 632 条 Term 的中文自由文本分类迁移为 28 个语义化 snake_case key，并按 9 个短名称分组集中维护；合并区块链、黑产生态和治理合规等过细分类，拆分原“业务安全”兜底分类。
- 新增中英文 Term 分类注册表，分类展示名统一翻译且中文标题最多 6 个汉字；英文 Term 文件不再逐实体重复维护 `category`。
- 术语列表支持按注册表顺序和分组筛选，列表徽标、详情、搜索、CLI、JSON-LD、静态数据包与 npm 类型均改为消费稳定分类 key 和本地化标签。
- `term-category-enum.mjs` 升级为阻断门禁，校验语义 key、注册表引用、分组关系、短名称、英文同步、未使用分类和逐实体英文污染；补充数据完整性测试及分类迁移脚本。
- 更新 Schema、数据模型、准入标准、LLM 分类评审、文档生成和实体版本工具；`entity:version:bump` 新增 `--type` 限定参数，便于在共享工作区安全地只处理指定实体类型。

## 2.44.0

完善威胁行为者体系与风险覆盖审计

- 新增物流套利团伙、恶意物流从业者、市场操纵者、恶意量化交易者、AI 智能体攻击者、平台规则滥用经营者和通信资源滥用服务商 7 个 ThreatActor，并同步英文翻译与权威来源。
- 全量复核 400 个 Risk 的威胁行为者归因，补齐营销作弊、供应链投毒、API 滥用、勒索、Web3、物流、资本市场、AI 智能体与身份认证等风险关系；删除边界过宽的身份伪造聚合角色，将关系迁回准确的现有角色。
- 新增非阻断命令 `npm run audit:risk-threat-actor-coverage` 与显式豁免清单，区分待补关系和合规、技术演进、功能安全等无明确攻击主体的 Risk；当前 383/400 个 Risk 已关联 ThreatActor，排除 17 个豁免后覆盖率为 100%。
- 运行 `sync:lateral-relations` 重算 ThreatActor/AttackTool 横向关系，自动递增受影响实体版本，并同步首页计数、README、贡献指南和 Skill 文档。
- 知识库数据分块增长后，将静态站性能门禁的资源数上限由 90 调整为 100；加载时间、总传输体积和脚本体积预算保持不变。
- 完善文档体系：新增架构与数据流水线、数据消费、发布维护双语指南，以及根目录贡献指南、安全报告流程和环境变量模板；修正版本命令、浏览器 CI、Case 路由、实体计数与 Entity Registry 说明，并增强文档语义一致性门禁。
...
- 「定义」「手段分类」「有效性」合并为同一 3 列网格（原定义独占一行、卡片在下一行），三者水平并排。
- 网格列宽改为 `2fr 1fr 1fr`，定义列更宽（约 2 倍），分类/有效性列等宽。
- 分类卡片标题由「规避手段」改为「手段分类」（新增 `avoidanceCategory` i18n key，中英文）。
