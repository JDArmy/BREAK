...
现有校验（`validate:data` 28 道门禁 + `data-integrity.test.ts`）已覆盖结构完整性、命名/ID/枚举、交叉引用、孤儿检测、keywords 含 title、i18n 同步、BD 归类、riskAssessment、title 去重/格式、updated 同步、ID 连续性、套话短语、Case.category 域名一致性、Risk.complexity AC 覆盖、Case.summary 关联交叉、Term.category 语义 key、注册表引用及 6 汉字分类名限制、实体粒度初筛。本标准补 6 个准入空白：
