...
// 用户手动切换起点类型时清空起点实体。用 computed setter 拦截 v-model 写入：
// 用户切 startType → setter 设 startType + 清空 startKey；
// assembly 根节点同步直设 pathExplorerStartType.value（不触发 setter，不清空 startKey）。
// 这样消除 syncFromRoot flag 与 watch(startType) 对 assembly 同步路径的依赖（修复切视角回 pathExplorer 时 startKey 被误清空）。
const startTypeModel = computed({
  get: () => pathExplorerStartType.value,
  set: (type: RelationType) => {
